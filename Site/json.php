...
$data3= $bdd->query('SELECT identifiant from user where pseudo =\''.$_SESSION['user'].'\'');
$data3->execute(); 
$data3 = $data3->fetch(PDO::FETCH_ASSOC); 
$data = $bdd->query('SELECT numpers AS `id`, groupe AS `group`, prenom AS `prename`, nom AS `name` FROM personne where numpers = '.$data3['identifiant']); // on Récupere la liste des personnes
